...
impl<'a> SSD1306<'a> {
    ...
    fn my_square(&mut self, x: usize, y: usize, w: usize, h: usize, color: bool) {
        ...
        if (w + x as isize) >= self.width as isize {
            w = self.width as isize - x as isize;
            if w <= 0 {
                return;
            }
        }
        if (h + y as isize) >= self.height as isize {
            h = self.height as isize - y as isize;
            if h <= 0 {
                return;
            }
        }
