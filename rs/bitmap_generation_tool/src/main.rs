...
use std::io::Write;
#[derive(Parser, Debug)]
#[clap(author="fx",version="0.1",about="SSD1306 bitmap pre-rotated converter",long_about = None)]
struct Config {
    /// input image
    #[clap(short, long)]
    image: String,
...
fn save_bitmap(
    ...
    write!(ofile, "// Pre rotated image converter for SSD1306 \n").expect("oops");
    write!(
        ofile,
...
fn main() {
    ...
    if args.image.is_empty() || args.output.is_empty() {
        Config::command().print_help().expect("oops");
        return;
    }
    ...
    for page in 0..(height / 8) {
        ...
        for column in 0..width {
            let mut byte: u8 = 0;
            for line in 0..8 {
                let pixel = img.get_pixel(column, y + line);
    ...
    if args.shrink {
        ...
        match heatshrink_byte::encode(&buffer, &mut output, &cfg) {
            Err(_x) => {
                panic!("Shrinking eror!");
            }
            Ok(x) => {
